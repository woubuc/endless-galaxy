...
export default class TopBarDate extends Vue {
	...
	get progressToNextTick(): number {
		let timeUntilNextTick = this.gameState.next_tick - (this.time / 1000);
		if (timeUntilNextTick <= 0) {
			return 0;
		}

		let timeSinceLastTick = (this.time / 1000) - this.gameState.last_tick;
		let tickTime = timeSinceLastTick + timeUntilNextTick;

		let progress = timeUntilNextTick / tickTime;
		if (progress <= 0) {
			return 0;
		}
